...
/**
 * 翻译 Markdown 内容，保留格式
 * 按段落分割翻译，保留代码块、链接、图片等 Markdown 语法
 * @param {string} markdown - Markdown 内容
 * @param {string} targetLang - 目标语言
 * @returns {Promise<{translatedMarkdown: string, error?: string}>}
 */
export async function translateMarkdown(markdown, targetLang = null) {
    ...
    if (!markdown || typeof markdown !== 'string') {
        return { translatedMarkdown: markdown, error: null };
    }
    ...
    try {
        ...
        // 按双换行分割段落
        const paragraphs = protectedMd.split(/\n\n+/);
        const BATCH_SIZE = settings.translateConcurrency || 10;
        const translatedParagraphs = new Array(paragraphs.length).fill(null);
        ...
        for (let i = 0; i < paragraphs.length; i += BATCH_SIZE) {
            ...
            const promises = batch.map(async (para, batchIndex) => {
                ...
                // 跳过空段落、纯占位符、标题标记等
                if (!trimmed ||
                    trimmed.match(/^__[A-Z_]+_\d+__$/) ||
                    trimmed.match(/^#+\s*$/) ||
                    trimmed.match(/^[-*_]{3,}$/)) {
                    translatedParagraphs[i + batchIndex] = para;
                    return;
                }

                try {
                    const result = await translateFn(trimmed, lang);
                    if (!result.error && result.translatedText) {
                        translatedParagraphs[i + batchIndex] = result.translatedText;
                    } else {
                        translatedParagraphs[i + batchIndex] = para;
                    }
                } catch (e) {
                    console.warn(`Markdown translation failed for paragraph ${i + batchIndex}:`, e);
                    translatedParagraphs[i + batchIndex] = para;
                }
            });
        ...
        return { translatedMarkdown: result, error: null };
    } catch (error) {
        console.error("Markdown translation error:", error);
        return { translatedMarkdown: markdown, error: error.message };
    }
}
